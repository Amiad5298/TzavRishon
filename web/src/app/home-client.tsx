  ...
  const handleCategoryClick = async (categoryId?: string) => {
    if (categoryId) {
      ...
      try {
        ...
        // Get the questions for the session (pre-fetch to ensure session is ready)
        await practiceApi.getQuestions(response.data.sessionId);

        // Navigate to practice page with session ID in query params
        router.push(`/practice?sessionId=${response.data.sessionId}&type=${categoryId}`);
      } catch (err: any) {

...
const Practice: React.FC = () => {
  ...
  // Initialize practice session from URL params (when coming from homepage category cards)
  useEffect(() => {
    const sessionIdParam = searchParams.get('sessionId');

    if (sessionIdParam && !sessionId) {
      // Session was already started on the homepage, fetch the questions
      setSessionId(sessionIdParam);
      setLoading(true);
      practiceApi.getQuestions(sessionIdParam)
        .then(response => {
  ...
  }, [searchParams, sessionId]); // Run when search params change
